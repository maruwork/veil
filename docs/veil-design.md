...
| 語彙ルール markdown ミラー | `~/.veil/rules/` | AI-readable markdown surface / 遷移ミラー |
| 同期先リスト | `~/.veil/targets.json` | 同期対象ファイルのパス一覧 |
| sync script パス | `~/.veil/config.json` | `--add` 実行時に自動記録 |

`~/.veil/` はユーザーホームに固定。current phase では canonical route は SQLite、markdown rules は遷移ミラーとして引き継がれる。
...
AI との会話から英語・造語・略語を検出し、task close / 会話区切りの閉じ処理として current phase では SQLite canonical に記録し、その後 markdown ミラーを更新する。
...
8. `shared/tools/veil-db.py export-mirror` 相当で `~/.veil/rules/` ミラーを更新する
9. `shared/runtime/veil-sync.py` を実行して即時同期する

### 3-2. shared/runtime/veil-sync.py

SQLite canonical を優先し、必要時に `~/.veil/rules/` markdown ミラーを再生成してから、各 AI ツール設定ファイルへ参照明記を反映する。
...
1. current phase で DB canonical が存在する場合は `~/.veil/rules/` ミラーを再生成する
2. `~/.veil/rules/` 以下の全 `.md` を読む
...
1. SQLite canonical または `~/.veil/rules/*.md` ミラーから `- original → preferred` 形式の rule を読む
2. 入力文を走査する
...
2. ミラーの最終更新時刻を表示する
3. 同期対象の登録数と存在確認を表示する
4. 常に exit 0

**動作（--check）**

1. DB, rules/, targets.json, 各同期対象、skill ファイルの存在を確認する
...
`shared/tools/veil-profile-audit.py`、`shared/tools/veil-profile-export.py`、`shared/tools/veil-db.py`、`shared/tools/veil_rule_store.py` は mainline runtime ではなく support runtime である。`shared/tools/veil-db.py` と `shared/tools/veil_rule_store.py` は SQLite canonical の schema / import / readback / upsert / ミラー export を補助する。`shared/tools/veil-profile-audit.py`、`shared/runtime/veil-normalize.py`、`shared/runtime/veil-lint.py` はいずれも `--db` で SQLite source を読める。
...
- 書き込み:
  ...
  - `shared/tools/veil-db.py`: 指定した SQLite DB path、指定したミラーディレクトリ
  - `shared/tools/veil_rule_store.py`: helper として SQLite DB / ミラーディレクトリを扱う

**用途**

1. `shared/tools/veil-profile-audit.py`
   - level 分布と legacy flat rule の有無を可視化する
   - `--db` による SQLite source 読取に対応
2. `shared/runtime/veil-normalize.py`
   - `--db` による SQLite source 読取に対応
3. `shared/runtime/veil-lint.py`
   - `--db` による SQLite source 読取に対応
4. `shared/tools/veil-profile-export.py`
   - 現在の標準プロファイルを section-aware のまま domain profile pack として書き出す
...
新規追加時は preferred を確定し、canonical DB へ記録してからミラーを再生成する。

候補の意味は次の通り。

| 項目 | 意味 |
|---|---|
| p1 | 採用語。canonical route に入り、current phase ではミラーと同期対象にもなる |
| p2 | 候補2。保持されるが同期対象外 |
| p3 | 候補3。保持されるが同期対象外 |
...
現在の標準プロファイルは technical writing 向けとする。

現在の標準プロファイルを repo 外や別 bundle に切り出す時は、`shared/tools/veil-profile-export.py` で profile pack を作る。これは domain profile 分離のための support route であり、mainline の `capture -> normalize -> sync -> lint` 自体は変えない。
...
現在の標準プロファイルの棚卸しには `shared/tools/veil-profile-audit.py` を使う。これは read-only で、level 分布と legacy flat rule の残りを可視化する補助である。

domain profile を別配布単位として扱いたい場合は、`shared/tools/veil-profile-export.py` で現在の標準プロファイルを pack 化してから tuning や分岐を行う。
