...
CREATE TABLE vocab (
  ...
  p1         TEXT DEFAULT '',       -- 採用語（実際に変換に使われる語）
  p2         TEXT DEFAULT '',       -- 代替候補（p1 の次の優先候補）
  p3         TEXT DEFAULT '',       -- 予備候補（差し替え検討中）
...
| 列 | 役割 | 運用 |
|----|------|------|
| p1 | 採用語 | 実際に変換に使われる語。ポップアップで別候補を選ぶと自動的に p1 に繰り上がる |
| p2 | 代替候補 | p1 の次の優先候補。p1 と差し替えたい語を置く |
| p3 | 予備候補 | 差し替えを検討中の語。すぐには使わないが候補として残したいとき |
