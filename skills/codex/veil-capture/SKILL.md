---
name: veil-capture
description: Detect terms from AI conversation; present candidates with locale-appropriate preferred forms; record the selected form to SQLite canonical; regenerate the markdown mirror and HTML; sync.
---
...
3. Regenerate the HTML using `python shared/tools/veil-db.py export-html`.
4. Check `sync_script` in `~/.veil/config.json`; if present, run `shared/runtime/veil-sync.py`.
