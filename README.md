...
- 迷う語は保留に回す。固有名か一般語か迷う語、別訳が衝突する語、まだ困り度が低い語は急いで正本化しない
...
```
採用:
- common asset → 共通資産 | 補助候補: コモンアセット, 共通アセット
- current state → 今の状態
- validator → バリデータ | 補助候補: 検査
...
- **候補1**：採用語。canonical route に記録され、`~/.veil/rules/` mirror を生成して AI ツール設定ファイルへ同期される
- **補助候補**：候補2 / 候補3 を本当に残す時だけ表示する。同期はされない
