...
| データ | 場所 | 用途 |
|--------|------|------|
| 語彙ルール（正本） | `~/.veil/rules/` | veil-capture が書き込む。clone先を変えても引き継がれる |
| 語彙DB | `{repodir}/vocab.db` | Web UI 用。`.gitignore` 済み。clone先変更後は自動再作成 |

**変換ポップアップの動作**

変換結果をクリックして別候補を選ぶと、**表示が切り替わるだけでなく語彙DBの優先順も更新**されます（選んだ語が p1 に繰り上がり）。次回以降の変換に即時反映されます。
