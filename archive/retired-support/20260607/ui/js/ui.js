...
function toggleSort() {
  sortBy = sortBy === 'freq' ? 'id' : sortBy === 'id' ? 'audit' : 'freq';
  document.getElementById('btn-sort').textContent =
    sortBy === 'freq' ? t('sortFreq') : sortBy === 'id' ? t('sortId') : t('sortAudit');
...
function onAuditFilter(val) {
  auditFilter = val;
  renderList(document.getElementById('search-input').value);
}

function getDropCandidateItems() {
  return Object.values(auditMap)
    .filter(item => item.status === 'drop-candidate');
}

function getReviewItems() {
  return Object.values(auditMap)
    .filter(item => item.status === 'review')
    .sort((a, b) => (a.use_count - b.use_count) || (a.id - b.id));
}

function setCurrentReview(item) {
  currentReviewId = item?.id ?? null;
  currentReviewOriginal = item?.original || '';
...
function reconcileCurrentReview() {
  if (!currentReviewId) return;
  const item = vocab.find(v => v.id === currentReviewId);
  const audit = item ? auditMap[currentReviewId] : null;
  if (!item || !audit || audit.status !== 'review') {
    clearCurrentReview();
    return;
  }
  currentReviewOriginal = item.o;
  renderCurrentReviewBanner();
}

function syncCurrentReviewWithOrigInput(value) {
  if (!currentReviewId) return;
  if ((value || '').trim() === currentReviewOriginal) return;
  clearCurrentReview();
}

function renderCurrentReviewBanner() {
  const el = document.getElementById('current-review');
  if (!el) return;
  if (!currentReviewId || !currentReviewOriginal) {
    el.style.display = 'none';
    el.textContent = '';
    return;
  }
  el.style.display = '';
  el.textContent = t('currentReviewLabel', currentReviewOriginal);
}

function fillReviewForm(item) {
  document.getElementById('orig').value = item.original;
  document.getElementById('pref1').value = item.p1 || '';
  document.getElementById('pref2').value = item.p2 || '';
  document.getElementById('pref3').value = item.p3 || '';
  document.getElementById('cat').value = item.cat || 1;
  document.getElementById('pref1').focus();
}

function getNextReviewItem() {
  return getReviewItems()[0] || null;
}
...
function focusNextReview() {
  ...
  fillReviewForm(item);
  showError(t('reviewNextDone', item.original));
}

function getDropCandidateIds() {
  return getDropCandidateItems().map(item => item.id);
}

function buildBulkDropPreview(items) {
  const names = items.map(item => item.original);
  const head = names.slice(0, 3);
  const rest = names.length - head.length;
  return rest > 0 ? `${head.join(' / ')} / 他 ${rest} 件` : head.join(' / ');
}

async function bulkDeleteDropCandidates() {
  const items = getDropCandidateItems();
  const ids = items.map(item => item.id);
  if (!ids.length) return;
  if (!window.confirm(t('bulkDropConfirm', ids.length, buildBulkDropPreview(items)))) return;
  const deleted = await deleteVocabBatch(ids);
  showError(t('bulkDropDone', deleted));
}
...
function applyLocale() {
  ...
  setText('lbl-audit', 'sectionAudit');
  ...
  const afMap = { 'all': 'auditAll', 'review': 'auditReview', 'drop-candidate': 'auditDrop', 'keep': 'auditKeep' };
  Array.from(document.getElementById('audit-filter').options).forEach(o => { if (afMap[o.value]) o.text = t(afMap[o.value]); });
  const reviewBtn = document.getElementById('btn-review-next');
  if (reviewBtn) reviewBtn.textContent = t('reviewNext', auditSummary.review || 0);
  const bulk = document.getElementById('btn-drop-bulk');
  if (bulk) bulk.textContent = t('bulkDrop', auditSummary['drop-candidate'] || 0);
  renderCurrentReviewBanner();
